...
import { initState, scoreEvent } from "../src/salience.js";
...
function stateFile(sid) { return path.join(dataDir(), "sessions", `${sid}.salience.json`); }
function loadState(sid) {
    try {
        return JSON.parse(fs.readFileSync(stateFile(sid), "utf8"));
    }
    catch {
        return initState();
    }
...
function main() {
    ...
    try {
        ...
        const ev = h.hook_event_name === "UserPromptSubmit"
        ...
        if (r.marker)
            appendEvent(sid, r.marker);
        if (r.pending)
            fs.writeFileSync(path.join(dataDir(), "sessions", `${sid}.pending`), "1");
        process.exit(0);
    }
    catch {
        process.exit(0);
