...
async function main() {
    if (!depsPresent(pluginRoot())) {
        try {
            writeFailure("distill skipped: dependencies not yet installed (bootstrap pending)");
        }
        catch { /* noop */ }
        try {
            releaseLock("distill");
...
if ((process.argv[1] && process.argv[1].endsWith("sb-distill.ts")) || process.argv[1]?.endsWith("sb-distill.js"))
    main();
