...
export function appendCandidate(vaultDir, c) {
 ...
 */
export function isPromotable(candidate, count) {
    if (count < 3)
        return false;
    ...
    if (!IMPERATIVE_PREFIXES.some(p => firstWord === p || firstTwo === p))
        return false;
    // Reject project-scoped: "for <name>: ..."
    if (/^for [\w-]+:/i.test(rule))
        return false;
    // Reject stack/tool-scoped: "when writing <X>" or "when using <X>"
    if (/^when\s+\w+\s+\w/i.test(rule))
        return false;
    return true;
}
 ...
 */
export function autoPromoteCandidates(vaultDir) {
    ...
    for (const [, entry] of counts) {
        if (!isPromotable({ rule: entry.display }, entry.count))
            continue;
    ...
    if (updated !== text)
        fs.writeFileSync(candFile, updated);
    return promoted;
}
