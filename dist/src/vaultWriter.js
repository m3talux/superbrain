        ...
        try {
            ...
            if (r.archived.length > 0) {
                ...
                for (const a of sortedArchived) {
                    ...
                    const archivePath = path.join(path.resolve(vaultPath()), "projects", "_archive", `${noteSlug}-${year}-Q${q}.md`);
                    const block = `\n### ${a.date}\n\n${a.content}\n`;
                ...
                for (const [archivePath, blocks] of byArchiveFile) {
                    ...
                    if (fs.existsSync(archivePath)) {
                        ...
                        existing_archive = serializeNote({ type: "summary", project: noteSlug, archived_from: `projects/${noteSlug}.md` }, `# ${noteSlug} - archive ${yearLabel}\n`);
                    }
            ...
            writeFailure(`vaultWriter fail-open fallback for ${rel}: ${_e?.message ?? String(_e)}`);
