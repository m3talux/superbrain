    ...
    try {
        if (currentProjectKnown && currentProjectSlug) {
            ...
            if (projectHits.length) {
                ...
                if (sid) {
                    try {
                        appendInjectedSlugs(sid, projectHits.map((p) => p.relPath));
                    }
                    catch { /* best-effort */ }
                }
            }
    ...
    try {
        ...
        if (globalQuery) {
            ...
            if (globalHits.length) {
                const injected = sid ? new Set(getInjectedSlugs(sid)) : new Set();
                ...
                if (dedupedHits.length) {
                    ...
                    if (sid) {
                        try {
                            appendInjectedSlugs(sid, dedupedHits.map((p) => p.relPath));
                        }
                        catch { /* best-effort */ }
                    }
                }
    ...
    try {
        ...
        if (currentProjectKnown && currentProjectSlug) {
            for (const s of Object.keys(day)) {
                ...
                if (entryProject !== undefined && entryProject !== currentProjectSlug)
                    continue;
                for (const t of entry.openThreads)
