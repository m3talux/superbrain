...
export function loadTokenizerSync(tokenizerJsonPath) {
    const raw = fs.readFileSync(tokenizerJsonPath, "utf-8");
    const parsed = JSON.parse(raw);
    const vocab = parsed.model.vocab;
    const unkToken = parsed.model.unk_token ?? "[UNK]";
    return {
        vocab,
        ...
        specialTokens: new Set((parsed.added_tokens ?? []).map((t) => t.id)),
    };
}
function normalizeText(text, doLowerCase) {
    let s = text
...
function wordpieceTokenize(word, tok) {
    if (word.length > tok.maxInputCharsPerWord)
        return [tok.unkTokenId];
    const subTokens = [];
    let start = 0;
