 ...
 */
import { loadTokenizerSync, tokenizeSync } from "./tokenizer.js";
...
let cached = null;
export function loadModelSync(tokenizerPath, safetensorsPath) {
    const tok = loadTokenizerSync(tokenizerPath);
    const tensors = loadSafetensorsSync(safetensorsPath);
    const key = Object.keys(tensors).find((k) => k === "embeddings" || k.includes("embeddings.weight") || k.includes("embedding.weight"));
    if (!key)
        throw new Error(`No embeddings tensor found. Keys: ${Object.keys(tensors).join(", ")}`);
    const matrix = tensors[key];
    const vocabSize = Object.keys(tok.vocab).length;
    const dim = matrix.length / vocabSize;
    if (!Number.isInteger(dim))
        throw new Error(`Matrix size ${matrix.length} not divisible by vocab size ${vocabSize}`);
...
export function embedWithModel(texts, model) {
