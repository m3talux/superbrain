...
function doAdopt(target: string): void {
  ...
  if (fs.existsSync(mk)) {
    let owned = false;
    try { owned = fs.statSync(mk).isFile() && fs.readFileSync(mk, "utf8").includes("superbrain-owned"); } catch { owned = false; }
    if (!owned) throw new Error(`adopt: ${abs} is marked by another tool`);
  }
  markOwned(abs);
