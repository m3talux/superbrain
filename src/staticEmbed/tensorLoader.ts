...
export function loadSafetensorsSync(filePath: string): TensorMap {
  ...
  try {
    ...
    for (const [name, info] of Object.entries(header) as [string, any][]) {
      ...
      const { dtype, data_offsets } = info as { dtype: string; shape: number[]; data_offsets: [number, number] };
      if (dtype !== "F32") throw new Error(`Unsupported dtype ${dtype} for tensor ${name}`);
