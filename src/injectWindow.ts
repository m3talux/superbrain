...
import { truncateToBudget, INJECT_LIMITS, PER_NOTE_TOKEN_CAP, estimateTokens } from "./injectBudget.js";
import { parseNote } from "./frontmatter.js";

/** The env-overridable period for mini-brief injection (every N turns). */
export const MINI_BRIEF_EVERY: number = (() => {
 ...
 */
export function capNoteExcerpt(excerpt: string, cap: number = PER_NOTE_TOKEN_CAP): string {
 ...
 */
export function readPreferencesCore(): string {
  try {
...
interface MiniBriefOpts {
  includePrefsCore?: boolean;
}
 ...
 */
export function buildMiniBrief(
  _sid: string,
  ...
  try {
    ...
    const title = parsed.data.title as string | undefined;
    const firstLine = parsed.content.split("\n").find((l) => l.trim().length > 0);
    if (title || firstLine) parts.push(`Today: ${title ?? firstLine}`);
  } catch { /* best-effort */ }
 ...
 */
export function shouldFireMiniBrief(turnCount: number): boolean {
